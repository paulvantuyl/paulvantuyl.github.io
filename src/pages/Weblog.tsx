...
export function Weblog() {
  ...
  const pageFileName =
    manifest?.pageFiles[currentPage - 1] ?? `posts-page-${currentPage}.json`
  ...
  return (
    <Layout title="Weblog" subtitle={subtitle}>
      <section className="filters">
        <div className="mt-2 mb-8 grid md:grid-cols-2 lg:grid-cols-4 items-start md:gap-3 gap-y-3 justify-stretch">
          ...
          <Select
            ...
            onChange={(value) => {
              setParams(
                { selectedCategory: String(value ?? 'all'), page: 1 },
                true,
              )
            }}
          ...
          {isLoadingAllPosts ? (
            <Text variant="p">Loading search index...</Text>
          ) : null}
        </div>
      </section>

      {isLoadingManifest || isLoadingPage ? (
        <Text variant="p">Loading posts...</Text>
      ) : null}
            ...
            {visiblePosts.map((post) => (
              <article className="blog-item" key={post.sourceFile}>
                ...
                <Text variant="h3" className="post-title">
                  <Link to={`/weblog/${post.slug}${location.search}`}>
                    {post.title}
                  </Link>
                </Text>
      ...
      <nav
        className="pagination mt-4 flex flex-row items-center gap-3"
